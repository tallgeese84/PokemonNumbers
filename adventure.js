...
function createAdventure() {
  ...
  const missionButton=document.createElement('button');missionButton.id='startAdventure';missionButton.className='widebtn';missionButton.textContent='⚡ Start today’s adventure';
  const missionNote=document.createElement('p');missionNote.className='adventure-mission-note';missionNote.textContent='A little warm-up, practice just for you, then a check-in.';
  document.querySelector('.modes').before(missionButton,missionNote);
  ...
  async function sync(cfg) {
    ...
    try {
      ...
      upload['devices/'+await deviceId()]={lastSeenAt:Date.now(),build:57,sessionId:tracker.sessionId};
