...
const PokeVisuals = (() => {
  ...
  function init() {
    ...
    document.getElementById('homeJournal').onclick=()=>document.getElementById('cloudBtn').click();
    ...
    for(const [key,[title,caption]] of Object.entries(activities)){
      ...
      head.innerHTML=`<span class="activity-icon">${icon(key==='quiz'?'count':key==='badges'?'badge':key)}</span><div><h2>${title}</h2><p>${caption}</p></div>`;
      if(['quiz','line','abacus'].includes(key)){
        const listen=document.createElement('button');listen.className='btn listen-btn';listen.textContent='↻ Listen';listen.setAttribute('aria-label','Hear the question again');
        listen.onclick=()=>document.getElementById({quiz:'qtext',line:'lnQ',abacus:'abQ'}[key]).click();head.append(listen);
      }
  ...
  function screen(name,mode){
    ...
    else document.querySelector('.topbar').after(timer);
    if(name==='quiz'){
      ...
      head.querySelector('.activity-icon').innerHTML=icon(mode);
    }
  }
