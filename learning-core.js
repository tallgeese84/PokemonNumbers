...
(function (root, factory) {
  ...
  class Tracker {
    constructor({now=()=>Date.now(), id=()=>Math.random().toString(36).slice(2), sessions={}, onChange=()=>{}}={}) {
      ...
      this.sessions[this.sessionId]={id:this.sessionId,startedAt:now(),updatedAt:now(),rev:0,days:{},questions:{},build:57};
    }
    ...
    begin(meta, ref) {
      ...
      if(this.lastPracticeAt && this.now()-this.lastPracticeAt>300000){
        ...
        this.sessions[this.sessionId]={id:this.sessionId,deviceId,startedAt:this.now(),updatedAt:this.now(),rev:0,days:{},questions:{},build:57};
